...
static bool ContainsMacro(const Preprocessor& preprocessor, const std::string& macroIdentifier)
{
	...
	return symTable.Contains(macroIdentifier);
}
...
TEST_CASE("Preprocessor Tests")
{
	...
	{
		std::string inputSource = R"(#include <system>
...
void main()
{
	return ADD(2, 3);
		...
		Preprocessor preprocessor(lexer, { errorCallback, [&currExpectedPathIndex, &expectedPaths](const std::string& path, bool isSystem)
		{
	...
	SECTION("TestProcess_PassSourceWithFunctionMacro_ReturnsProcessedSource")
	{
		...
		preprocessor.Process(); //std::cout << preprocessor.Process() << std::endl;
		REQUIRE(result);
	}
...
int main(int argc, char** argv) {
	...
	return -1;
		...
		REQUIRE_THAT(output, !Catch::Matchers::ContainsSubstring("COMMENT"));
	}
}